...
export default function HomePage() {
  return (
    <>
      <section className="mx-auto grid max-w-6xl gap-12 px-5 pb-16 pt-16 sm:pt-24 lg:grid-cols-[1.04fr_0.96fr] lg:items-center lg:px-8 lg:pb-24">
        <div>
          ...
          <p className="mt-6 max-w-xl text-lg leading-8 text-[var(--muted-foreground)]">
            The PMS knows the booking. Housekeeping knows the room. The messaging tool knows the guest is coming early. HOS is an open standard
            designed to let them share one picture, so your team sees problems before guests do, and tomorrow&apos;s AI tools work under your rules.
          </p>
          <div className="mt-8 flex flex-wrap gap-3">
            <Link href="/participate/pilot">
              <Button size="lg">
                Run a pilot <ArrowRight aria-hidden="true" size={17} />
              </Button>
            </Link>
            <Link href="/participate/founding-member">
      ...
      <SectionFrame
        eyebrow="The problem"
        title="A simple arrival decision is rarely simple."
        description="A guest writes to say they will arrive early. The PMS expects them this afternoon. Housekeeping still has the room as dirty. Each system holds part of the answer, and nobody sees the whole situation until the guest is at the front desk."
      >
        <div className="grid gap-px overflow-hidden rounded-lg border border-[var(--border)] bg-[var(--border)] md:grid-cols-3">
          {[
            [
              "Every system names things differently",
              "The same booking, guest and room carry a different reference in each system. Matching them takes a custom integration, every time.",
            ],
            [
              "Systems disagree",
              "The PMS shows the room as clean; housekeeping has not inspected it yet. Your team has to know which one to believe.",
            ],
      ...
      <SectionFrame
        eyebrow="First proof"
        title="See the problem before the guest does."
        description="HOS starts with one everyday situation: a guest arriving to a room that is not ready. It is tested on three cases: an early arrival, a room going out of order on the morning of arrival, and a late check-out in a room promised to someone else. HOS raises the alert and shows where each piece of information comes from. It never checks a guest in, moves a guest or changes a booking: your team stays in charge."
      >
        ...
        <div className="mt-6 flex flex-wrap gap-3">
          <Link href="/demo">
            <Button>
              Watch it step by step <ArrowRight aria-hidden="true" size={16} />
            </Button>
          </Link>
          <Link href="/docs/core">
            <Button variant="secondary">Read the technical specification</Button>
          </Link>
        </div>
      </SectionFrame>
          ...
          {[
            {
              ...
              text: "A booking, a room or a cleaning task means the same thing in every system that speaks HOS, and each piece of information says where it came from.",
            },
            {
              Icon: ShieldCheck,
              title: "You stay in charge",
              text: "Each hotel's data stays separate, each system states what it is responsible for, and anything that happens can be checked afterwards.",
            },
            {
              Icon: Workflow,
              title: "AI, one step at a time",
              text: "HOS starts by spotting problems. Suggestions come next, then actions your team approves. AI gets more freedom only once it has earned it.",
            },
          ].map(({ Icon, title, text }) => (
            <Card className="p-6" key={title}>
              <Icon aria-hidden="true" className="text-[var(--accent)]" size={21} />
      ...
      <SectionFrame eyebrow="Trust by design" title="Nothing hidden, nothing taken for granted.">
        <ul className="grid gap-3 md:grid-cols-2">
          {[
            ...
            <li className="flex gap-3 rounded-md border border-[var(--border)] p-4 text-sm leading-6" key={item}>
              <CircleCheck aria-hidden="true" className="mt-0.5 shrink-0 text-[var(--accent)]" size={17} />
              {item}
            </li>
          ))}
        </ul>
      ...
      <SectionFrame
        ...
        title="Find the role that fits your organisation."
        description="Early participants shape HOS around how hotels really work. Nobody gets exclusive rights over the standard, over anyone's data or over future certification."
      >
        <div className="grid gap-4 md:grid-cols-2">
          {participationPaths.map((path, index) => (
            <ParticipationCard {...path} featured={index === 0} key={path.slug} />
          ))}
        </div>
      </SectionFrame>
        ...
        <div className="mx-auto flex max-w-6xl flex-col justify-between gap-6 px-5 py-16 sm:py-20 lg:flex-row lg:items-end lg:px-8">
          <div>
            ...
            <h2 className="mt-3 max-w-2xl text-3xl font-semibold tracking-[-0.055em] sm:text-5xl">
              Help hotels and their software speak the same language.
            </h2>
          </div>
          <div className="flex flex-wrap gap-3">
            <Link href="/participate/pilot">
              <Button size="lg">Run a pilot</Button>
            </Link>
            <Link href="/docs">
              <Button size="lg" variant="secondary">
