...
export async function writeLiveReport(
  report: SyncReport<Record<string, number>>,
  {
    ...
    events,
    record,
  }: { heading: string; notes?: string[]; context: Record<string, unknown>; out?: string; events: boolean; record: string },
) {
  const summary = {
    ...context,
  ...
  const started = Object.entries(
    arrivals
      ...
      .reduce<Record<string, number>>((counts, stay) => ({ ...counts, [stay.stay_status]: (counts[stay.stay_status] ?? 0) + 1 }), {}),
  ).map(([status, total]) => `${total} ${status.replaceAll("_", " ")}`);
  console.log(heading);
  console.log(
    `Fetched ${Object.entries(report.fetched)
      .map(([entity, total]) => `${total} ${entity.replaceAll("_", " ")}`)
      .join(", ")}.`,
  );
  for (const note of notes) console.log(note);
  console.log(
    `HOS facts: ${report.events.length}, schema errors: ${report.schema_errors.length}, failures: ${report.failures.length}, facts on a second pass: ${report.resync_events}`,
  );
  ...
  const files = {
    manifest: path.join(record, "manifest.json"),
    stream: path.join(record, "recording.jsonl"),
    redelivery: path.join(record, "redelivery.jsonl"),
  };
  ...
  const problems = [...check.manifest.errors, ...check.recording.issues, ...(check.redelivery?.issues ?? [])].filter(
    (issue) => !("severity" in issue) || issue.severity === "error",
  );
  for (const problem of problems.slice(0, 10))
    console.log(`  ${"line" in problem && problem.line ? `line ${problem.line}: ` : ""}${problem.message}`);
  console.log(
    `Files in ${record}. Check them again with: hos conformance producer --manifest ${files.manifest} --stream ${files.stream} --redelivery ${files.redelivery}`,
  );
