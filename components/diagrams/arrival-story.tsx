...
export function EventFlowDiagram() {
  ...
  useEffect(() => {
    ...
    const starts = [0.025, 0.1, 0.19, 0.265, 0.35];
    ...
    figure.querySelectorAll("[data-story-caption]").forEach((caption, index) => {
      const start = [0, 0.4, 0.55][index];
      const end = [0.4, 0.55, 1][index];
      animate(caption, [
        { opacity: index === 0 ? 1 : 0, offset: 0 },
        ...(index > 0
          ? [
              { opacity: 0, offset: start },
              { opacity: 1, offset: start + 0.02 },
            ]
          : []),
    ...
    <figure ref={figureRef} className={styles.story} aria-label="How HOS spots an early-arrival risk">
      ...
      <header className={styles.header}>
        <div className={styles.topline}>
          <div className={styles.eyebrow}>
            <span className={styles.statusDot} /> A hotel morning
          </div>
          <div className={styles.buttons}>
            <button aria-label={showPlay ? "Play arrival animation" : "Pause arrival animation"} onClick={togglePlayback} type="button">
              {showPlay ? <Play aria-hidden="true" size={13} /> : <Pause aria-hidden="true" size={13} />}
              {showPlay ? "Play story" : "Pause"}
            </button>
            <button aria-label="Replay arrival animation" onClick={replay} type="button">
        ...
        <p className={styles.headline}>
          The guest is early.
          <br />
          <span>See the risk before they do.</span>
        </p>
      ...
      <div className={styles.sources}>
        <div className={styles.source} data-source-card>
          ...
          <div data-flow-event className={styles.smallFact}>
            <Check size={11} aria-hidden="true" /> Confirmed
          </div>
          <div data-flow-event className={styles.fact}>
            <strong>15:00</strong>
            <span>Check-in · room 204</span>
          </div>
        </div>
        <div className={styles.source} data-source-card>
          <div className={styles.sourceName}>
          ...
          <div data-flow-event className={styles.smallFact}>
            <span className={styles.oldStatus}>Room dirty</span>
            <ArrowRight size={11} aria-hidden="true" />
          </div>
          <div data-flow-event className={styles.fact}>
            <strong className={styles.wordFact}>Cleaned</strong>
            <span className={styles.pending}>Not inspected</span>
          </div>
        </div>
        <div className={`${styles.source} ${styles.guestSource}`} data-source-card>
          ...
          <div data-flow-event className={styles.message}>
            <span>“We’ll be there at</span>
            <strong>
              12:00<span>”</span>
            </strong>
            <span className={styles.early}>3 hours early</span>
          </div>
        </div>
      </div>
      ...
      <div className={styles.connection}>
        <svg className={styles.wires} viewBox="0 0 480 104" fill="none" preserveAspectRatio="none" aria-hidden="true">
          {["M78 0 V22 Q78 38 94 38 H220 Q240 38 240 58 V80", "M240 0 V80", "M402 0 V22 Q402 38 386 38 H260 Q240 38 240 58 V80"].map(
            (path, index) => (
              <g key={path}>
                ...
                <path
                  ...
                  d={path}
                  pathLength="1"
                  stroke={index === 2 ? "var(--story-amber)" : "var(--story-blue)"}
                  strokeWidth="2"
                  strokeDasharray="1"
                />
              </g>
        ...
        <div data-hos-hub className={styles.hub}>
          <HOSMark className="h-7 w-7" />
          <span>HOS</span>
        </div>
        <p data-hos-insight className={styles.insight}>
          The same room. The whole picture.
        </p>
      </div>

      <div className={styles.outcome}>
        <div data-pending-insight className={styles.pendingInsight} aria-hidden="true">
          <CircleAlert size={21} />
          <strong>The risk is still hidden.</strong>
          <span>No single tool has the whole picture.</span>
        </div>
        <div data-flow-alert className={styles.alert}>
          <div className={styles.alertEyebrow}>
            <CircleAlert size={15} aria-hidden="true" /> Early arrival · risk detected <span>204</span>
          </div>
          <p className={styles.alertTitle}>
            <span className="sr-only">Alert: </span>room 204 may not be ready in time
          </p>
          <p className={styles.alertDetail}>Expected at 12:00. Inspection still pending.</p>
          <div className={styles.authority}>
            <ShieldCheck size={14} aria-hidden="true" />
            <span>
              HOS alerts. <strong>Your team decides.</strong>
            </span>
          </div>
        </div>
      </div>
      ...
      <footer className={styles.footer}>
        <div className={styles.chapters} aria-hidden="true">
          {["The signals", "The connection", "The heads-up"].map((chapter, index) => (
            <div key={chapter}>
        ...
        <div className={styles.controls}>
          <span className={styles.duration}>
            Illustrative scenario <span>· 16 seconds</span>
          </span>
          <span className={styles.version}>HOS 0.1</span>
        </div>
      </footer>
      <figcaption className="sr-only">
        Illustrative early arrival for room 204. The PMS expects check-in at 15:00. Housekeeping reports the room dirty, then cleaned but not
        inspected. The guest expects to arrive at 12:00. HOS connects these facts and alerts the front desk that the room may not be ready. HOS does
        not change the booking or check the guest in; the team decides what to do.
      </figcaption>
    </figure>
  );
}
