...
export function LegalText({ value }: { value: LegalValue }) {
  ...
  return (
    <mark className="rounded-sm bg-[var(--warning-soft)] px-1 font-mono text-[0.85em] text-[var(--warning)]">[To complete: {value.pending}]</mark>
  );
}
  ...
  return (
    <>
      <section className="mx-auto max-w-3xl px-5 pb-8 pt-16 sm:pt-24 lg:px-8">
        ...
        {open ? (
          <div
            className="mt-6 border-l-2 border-[var(--warning)] bg-[var(--warning-soft)] p-4 text-sm leading-6 text-[var(--muted-foreground)]"
            role="note"
          >
            <p className="font-semibold text-[var(--foreground)]">Incomplete draft.</p>
            <p className="mt-1">
              {open === 1 ? "One publisher detail is" : `${open} publisher details are`} still to be completed across the legal pages. They are marked
              “To complete”.
            </p>
          </div>
        ) : null}
        <nav aria-label="On this page" className="mt-8 border-y border-[var(--border)] py-4">
          <ol className="grid gap-1 text-sm sm:grid-cols-2">
            {contents.map((item) => (
              <li key={item.id}>
                <a className="underline decoration-[var(--border-strong)] underline-offset-4 hover:decoration-[var(--accent)]" href={`#${item.id}`}>
                  {item.title}
                </a>
              </li>
            ))}
      ...
      <table className="w-full min-w-[36rem] border-collapse text-left text-sm">
        ...
        <thead>
          <tr className="border-b border-[var(--border-strong)]">
            {head.map((cell) => (
              <th className="py-2 pr-4 font-medium text-[var(--foreground)]" key={cell} scope="col">
                {cell}
              </th>
            ))}
          </tr>
        ...
        <tbody>
          {rows.map((row, index) => (
            <tr className="border-b border-[var(--border)] align-top" key={index}>
              {row.map((cell, cellIndex) => (
                <td className="py-2 pr-4" key={cellIndex}>
                  {cell}
                </td>
              ))}
            </tr>
