...
const expectOpacity = (element: Locator, value: string) => expect.poll(
  () => element.evaluate((node) => getComputedStyle(node).opacity),
  { intervals: [40] },
).toBe(value);
...
for (const reducedMotion of ["reduce", "no-preference"] as const) {
  test(`the story autoplays and loops until paused (${reducedMotion})`, async ({ page }) => {
    ...
    await expect.poll(() => rows.nth(0).evaluate(el => el.getAnimations()[0].effect?.getComputedTiming().currentIteration), { timeout: 10000 }).toBe(1);
...
test("the illustration follows light and dark themes without losing its paused position", async ({ page, isMobile }, testInfo) => {
  ...
  await figure.evaluate(el => Promise.all(el.getAnimations({ subtree: true }).map(animation => animation.ready)));
  await figure.evaluate(el => el.getAnimations({ subtree: true }).forEach(animation => { animation.currentTime = 11000; }));
  await expect(figure).toHaveCSS("background-color", "rgb(247, 250, 255)");
  const lightBorder = await page.locator("[data-source-card]").first().evaluate(el => getComputedStyle(el).borderColor);
  ...
  expect(await page.locator("[data-source-card]").first().evaluate(el => getComputedStyle(el).borderColor)).not.toBe(lightBorder);
  expect(await figure.evaluate(el => el.getAnimations({ subtree: true }).every(animation => animation.playState === "paused" && animation.currentTime === 11000))).toBe(true);
