...
describe("Apaleo live synchronisation", () => {
  ...
  const cancelled: ApaleoReservation = { ...assigned, id: "HOSXKQPT-2", status: "Canceled", cancellationTime: "2026-07-20T10:00:00+02:00", unit: undefined };
  const maintenance: ApaleoMaintenance = { id: "HOSX-PAR-MNT1", unit: { id: room.id, name: "204" }, from: "2026-07-30T12:00:00+02:00", to: "2026-07-30T18:00:00+02:00", type: "OutOfOrder" };
  ...
  it("maps a fetched property to valid facts, once", () => {
    ...
    expect(report.events.find((event) => event.type === "stay.unit_assigned")).toMatchObject({ time: "2026-07-30T06:06:00Z", hostimebasis: "modified" });
  ...
  it("reports today's arrivals, with a maintenance window as a risk", () => {
    ...
    expect(blocked.arrivals.stays).toEqual([
      expect.objectContaining({ unit: "204", housekeeping: "clean", readiness: "not_ready", situation: "at_risk", maintenance: { starts_at: "2026-07-30T10:00:00Z", ends_at: "2026-07-30T16:00:00Z" } }),
    ]);
