...
describe("Mews adapter", () => {
  ...
  function sync(target: ReturnType<typeof adapter>, change: Partial<MewsReservation>, received_at = change.UpdatedUtc ?? reservation.UpdatedUtc) {
    const result = target.handle({
      received_at,
      webhook: webhook("ServiceOrderUpdated", reservation.Id),
  ...
  function sense(target: ReturnType<typeof adapter>, change: Partial<MewsResource>) {
    const result = target.handle({
      received_at: change.UpdatedUtc ?? room.UpdatedUtc,
      webhook: webhook("ResourceUpdated", room.Id),
      resources: [{ ...room, ...change }],
    });
    for (const event of result.events) expect(validateEvent(event), JSON.stringify(validateEvent.errors)).toBe(true);
    return result;
  }
  ...
  it("waits for a commitment before publishing a reservation", () => {
    ...
    expect(sync(target, { State: "Confirmed", UpdatedUtc: "2026-07-13T08:00:00Z" }).events).toMatchObject([
      { type: "reservation.updated", data: { changed_fields: ["status"], status: "confirmed" } },
    ]);
  });
  ...
  it("maps a no-show to a status update and other cancellations to reservation.cancelled", () => {
    ...
    sync(noShow, {});
    expect(
      sync(noShow, { State: "Canceled", CancellationReason: "NoShow", CancelledUtc: "2026-07-31T02:00:00Z", UpdatedUtc: "2026-07-31T02:00:00Z" })
        .events,
    ...
    expect(
      sync(cancelled, {
        ...
        CancelledUtc: "2026-07-20T10:00:00Z",
        UpdatedUtc: "2026-07-20T10:00:00Z",
      }).events,
    ).toMatchObject([{ type: "reservation.cancelled", data: { reason: "guest_request" } }]);
  ...
  it("keeps the assignment history, releases included, dated by the reservation's last update", () => {
    const target = adapter();
    sync(target, {});
    expect(sync(target, assigned).events).toMatchObject([
      { type: "stay.unit_assigned", hostimebasis: "modified", data: { unit_id: "unit_204", previous_unit_id: null, reason: "initial_assignment" } },
    ]);
    ...
    expect(released).toMatchObject([
      {
        ...
        data: { unit_id: (moved[0].data as { unit_id: string }).unit_id },
      },
    ]);
  ...
  it("publishes a missed check-in before the check-out", () => {
    ...
    const departed = sync(target, {
      AssignedResourceId: room.Id,
      State: "Processed",
      ActualStartUtc: "2026-07-30T12:40:00Z",
      ActualEndUtc: "2026-08-01T08:10:00Z",
      UpdatedUtc: "2026-08-01T08:10:00Z",
    }).events;
  ...
  it("keeps each Mews resource state on its HOS dimension", () => {
    const target = adapter();
    expect(sense(target, { State: "OutOfOrder", UpdatedUtc: "2026-07-29T09:00:00Z" }).events).toMatchObject([
      { data: { dimension: "maintenance", current: "out_of_service" } },
    ]);
    ...
    expect(sense(target, { State: "Inspected", UpdatedUtc: "2026-07-30T10:10:00Z" }).events).toMatchObject([
      { data: { dimension: "housekeeping", previous: "dirty", current: "inspected" } },
    ]);
  });

  it("maps resource blocks to maintenance windows, and a deleted block to a cancellation", () => {
    const target = adapter();
    const base: MewsResourceBlock & { Name: string; Notes: string } = {
      ...
      Notes: "Guest complained about the noise",
    };
    const block = (change: Partial<MewsResourceBlock>) => {
      const result = target.handle({
        received_at: change.UpdatedUtc ?? base.UpdatedUtc,
        webhook: webhook("ResourceBlockUpdated", base.Id),
        resourceBlocks: [{ ...base, ...change }],
      });
      for (const event of result.events) expect(validateEvent(event), JSON.stringify(validateEvent.errors)).toBe(true);
      return result;
    };
    ...
    expect(moved).toMatchObject([
      {
        hostimebasis: "modified",
        data: {
          maintenance_id: (planned.data as { maintenance_id: string }).maintenance_id,
          statuses: { commercial: "not_sellable" },
          reason: "internal_use",
        },
      },
    ]);
    expect(block({ IsActive: false, DeletedUtc: "2026-07-29T12:00:00Z", UpdatedUtc: "2026-07-29T12:00:00Z" }).events).toMatchObject([
      { type: "unit.maintenance_cancelled", time: "2026-07-29T12:00:00Z", data: { unit_id: "unit_204" } },
    ]);
    expect(block({ IsActive: false, DeletedUtc: "2026-07-29T12:00:00Z", UpdatedUtc: "2026-07-29T12:00:00Z" }).unmapped[0].reason).toBe(
      "No change since the last fetch.",
    );
  });

  it("leaves other services, other enterprises and customer profiles out of HOS", () => {
    const target = adapter();
    expect(sync(target, { ServiceId: "6015cefd-5347-4a04-be80-1f1931d58c6b" }).unmapped[0].reason).toBe(
      "Not an accommodation service at this property.",
    );
    const stranger = target.handle({
      received_at: reservation.UpdatedUtc,
      webhook: webhook("ServiceOrderUpdated", reservation.Id, "00000000-0000-4000-8000-000000000000"),
      reservations: [reservation],
    });
    expect(stranger).toEqual({
      events: [],
      unmapped: [{ event: "ServiceOrderUpdated", id: reservation.Id, reason: "The enterprise is not configured as a HOS property." }],
    });
