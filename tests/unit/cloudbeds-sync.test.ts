...
describe("Cloudbeds live synchronisation", () => {
  ...
  const block: CloudbedsRoomBlock = {
    roomBlockID: "RB-311",
    roomBlockType: "out_of_service",
    roomBlockReason: "Leaking shower",
    startDate: "2026-07-30",
    endDate: "2026-07-30",
    rooms: [{ eventID: "EV-1", roomID: room.roomID! }],
  };
  const hold: CloudbedsRoomBlock = {
    ...block,
    roomBlockID: "RB-312",
    roomBlockType: "courtesy_hold",
    rooms: [{ eventID: "EV-2", roomID: "418204-15" }],
  };
  const snapshot = (roomBlocks: CloudbedsRoomBlock[]): CloudbedsSnapshot => ({
    fetchedAt: "2026-07-30T08:30:00Z",
    property: {
      id: property.cloudbedsPropertyId,
  ...
  it("maps a fetched property to valid facts, once", () => {
    ...
    expect(report.room_blocks).toEqual([
      {
        ...
        end_date: "2026-07-30",
        rooms: [{ room: "204", starts_at: "2026-07-30T13:00:00Z", ends_at: "2026-07-31T09:00:00Z" }],
      },
    ...
    for (const type of ["reservation.created", "stay.unit_assigned"])
      expect(report.events.find((event) => event.type === type)).toMatchObject({ time: "2026-07-30T08:30:00Z", hostimebasis: "recorded" });
  ...
  it("reports today's arrivals, with a room block as a risk", () => {
    ...
    expect(blocked.arrivals.stays).toEqual([
      expect.objectContaining({
        ...
        readiness: "not_ready",
        situation: "at_risk",
        maintenance: { starts_at: "2026-07-30T13:00:00Z", ends_at: "2026-07-31T09:00:00Z" },
      }),
    ]);
  ...
  it("isolates a reservation the adapter cannot read", () => {
    const broken: CloudbedsReservation = {
      ...assigned,
      reservationID: "5830021044",
      startDate: "not a date",
      assigned: [{ ...assigned.assigned![0], subReservationID: "5830021044-1", startDate: "not a date" }],
    };
