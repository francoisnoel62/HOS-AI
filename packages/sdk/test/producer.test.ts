...
const errorRules = (issues: Array<{ severity?: string; rule?: string }>) => issues.filter((issue) => issue.severity === "error").map((issue) => issue.rule);
...
describe("checkProducer", () => {
  it("passes a producer whose facts are valid, its own and declared", () => {
    ...
    expect(check).toMatchObject({ valid: true, producer: "urn:hos:pms:demo", manifest: { valid: true, errors: [] }, recording: { valid: true, events: facts.length } });
  ...
  it("passes a redelivery of the same facts with the same ids, even recorded again", () => {
    ...
    expect(checkProducer({ manifest: pms, recording: streamText(facts), redelivery: streamText(again) })).toMatchObject({ valid: true, redelivery: { valid: true, events: facts.length, repeated: facts.length } });
    expect(checkProducer({ manifest: pms, recording: streamText(facts), redelivery: "" })).toMatchObject({ valid: true, redelivery: { events: 0, repeated: 0 } });
  });
  ...
  it("fails a redelivery that brings a new id or other content for an id", () => {
    ...
    expect(redelivery!.issues).toMatchObject([
      { line: 1, rule: "events/at-least-once-delivery", message: expect.stringContaining(`${renamed.id} from urn:hos:pms:demo is not in the recording`) },
  ...
  it("reports a fact under another source once, as such", () => {
    ...
    expect(valid).toBe(false);
    expect(recording.issues).toMatchObject([{ line: facts.length + 1, path: "/source", rule: "events/producers", message: expect.stringMatching(/^source is urn:hos:housekeeping:demo, but the manifest is urn:hos:pms:demo's\./) }]);
  });
  ...
  it("fails a fact its manifest does not declare, and an empty recording", () => {
    ...
    expect(errorRules(checkProducer({ manifest: pms, recording: streamText([undeclared]) }).recording.issues)).toEqual(["events/declared-capability"]);
    expect(checkProducer({ manifest: pms, recording: "" }).recording.issues).toMatchObject([{ severity: "error", message: "The recording holds no fact." }]);
  });
  ...
  it("checks what a manifest says of snapshots", () => {
    ...
    expect(errorRules(checkProducer({ manifest: silent, recording: streamText([snapshot]) }).recording.issues)).toEqual(["events/explicit-snapshots"]);
  });
});
