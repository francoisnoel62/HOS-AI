...
async function signWithHeader(header: Record<string, unknown>, privateJwk: JWK, payload = canonicalize(manifest)) {
  const jws = await new CompactSign(new TextEncoder().encode(payload)).setProtectedHeader(header as never).sign(await importJWK(privateJwk, privateJwk.alg));
...
describe("canonicalize", () => {
  it("writes the example of RFC 8785", () => {
    const text = '{"numbers": [333333333.33333329, 1E30, 4.50, 2e-3, 0.000000000000000000000000001], "string": "\\u20ac$\\u000F\\u000aA\'\\u0042\\u0022\\u005c\\\\\\"\\/", "literals": [null, true, false]}';
    expect(canonicalize(JSON.parse(text))).toBe('{"literals":[null,true,false],"numbers":[333333333.3333333,1e+30,4.5,0.002,1e-27],"string":"€$\\u000f\\nA\'B\\"\\\\\\\\\\"/"}');
  });
...
describe("the signing test vectors", () => {
  it("cover each verdict the plan names", () => {
    expect(vectors.map((vector) => vector.file)).toEqual(
      expect.arrayContaining(["valid-ed25519.json", "valid-es256.json", "expired.json", "wrong-key.json", "modified-manifest.json", "unknown-kid.json", "alg-none.json"]),
    );
  ...
  it.each(vectors.filter((vector) => vector.expected.valid).map((vector) => [vector.file, vector] as const))("%s verifies with node:crypto alone", (_, vector) => {
    ...
    const ok = header.alg === "ES256" ? verify("sha256", input, { key, dsaEncoding: "ieee-p1363" }, Buffer.from(signature, "base64url")) : verify(null, input, key, Buffer.from(signature, "base64url"));
    expect(ok).toBe(true);
...
describe("the /.well-known/hos/ example", () => {
  it("is a valid manifest, signed with the key its producer publishes", async () => {
    ...
    expect(result).toMatchObject({ valid: true, header: { alg: "Ed25519", kid: example.jwks.keys[0].kid, iat: seconds(new Date("2026-09-26T00:00:00Z")), exp: seconds(new Date("2027-09-26T00:00:00Z")) } });
  });
});
...
describe("signManifest and verifyManifest", () => {
  ...
  it("allows 60 seconds of clock skew, and no more", async () => {
    ...
    const jws = await signManifest(manifest, privateJwk, { issuedAt, expiresAt });
    const at = (date: Date, offset: number) => verifyManifest(manifest, jws, { keys: [publicJwk] }, { now: new Date(date.getTime() + offset * 1000) });
    expect(await at(expiresAt, 59)).toMatchObject({ valid: true });
    expect(await at(expiresAt, 60)).toMatchObject({ valid: false, error: "expired", message: "The signature expired on 2026-12-30T00:00:00.000Z. The producer signs its manifest again before it expires." });
  ...
  it("signs only a valid manifest, with a private key that has its kid", async () => {
    const { privateJwk, publicJwk } = await generateManifestKey();
    await expect(signManifest({ ...manifest, limitations: [""] }, privateJwk, { expiresAt })).rejects.toThrow("Only a valid producer manifest is signed: /limitations/0");
  ...
  it("refuses a header without kid, iat or exp, or with crit or b64", async () => {
    ...
    expect(await verdict({ kid: publicJwk.kid, iat: times.exp, exp: times.iat })).toBe("malformed: exp is not after iat: the signature expires before it is made.");
