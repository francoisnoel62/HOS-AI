...
describe("hos manifest keygen", () => {
  it("writes a private key and a key set with its public key", async () => {
    ...
    expect(code).toBe(0);
    expect(JSON.parse(written["key.json"])).toMatchObject({ kid: "2026-10", kty: "OKP", crv: "Ed25519", d: expect.any(String), alg: "Ed25519", use: "sig" });
  ...
  it("adds a new key to an existing key set, to rotate keys", async () => {
    ...
    const second = await hos(["manifest", "keygen", "--key", "new.json", "--jwks", "jwks.json", "--kid", "2026-12", "--alg", "ES256"], { files: { "jwks.json": first.written["jwks.json"] } });
    expect(second.code).toBe(0);
    expect(JSON.parse(second.written["jwks.json"]).keys.map((key: { kid: string; kty: string }) => `${key.kid} ${key.kty}`)).toEqual(["2026-09 OKP", "2026-12 EC"]);
  ...
  it("never overwrites a key, nor reuses a kid", async () => {
    ...
    expect(existing).toMatchObject({ code: 2, written: {}, stderr: expect.stringContaining("key.json already exists. A new key goes to a new file, so no key is lost.") });
    const first = await hos(["manifest", "keygen", "--key", "a.json", "--jwks", "jwks.json", "--kid", "k1"]);
    const again = await hos(["manifest", "keygen", "--key", "b.json", "--jwks", "jwks.json", "--kid", "k1"], { files: { "jwks.json": first.written["jwks.json"] } });
    expect(again).toMatchObject({ code: 2, written: {}, stderr: expect.stringContaining("jwks.json already has a key with kid k1. A kid names one key.") });
  });
...
describe("hos manifest sign and verify", () => {
  it.each(["Ed25519", "ES256"])("sign with %s a manifest that the SDK and hos verify", async (alg) => {
    ...
    expect(signed.stdout).toMatch(new RegExp(`^Signed manifest\\.json with key ${jwks.keys[0].kid} \\(${alg}\\), until \\d{4}-\\d\\d-\\d\\dT[\\d:]+Z: manifest\\.jws\\.\\n`));
    expect(await verifyManifest(JSON.parse(manifestText), files["manifest.jws"], jwks)).toMatchObject({ valid: true, header: { alg, kid: jwks.keys[0].kid } });
  ...
  it("refuses to sign a manifest that fails the checks", async () => {
    ...
    const { code, stdout, written } = await hos(["manifest", "sign", "manifest.json", "--key", "key.json"], { files: { ...files, "manifest.json": invalid } });
    expect(code).toBe(1);
  ...
  it("refuses to sign with a public key", async () => {
    ...
    const { code, stderr } = await hos(["manifest", "sign", "manifest.json", "--key", "public.json"], { files: { ...files, "public.json": JSON.stringify(publicKey) } });
    expect(code).toBe(2);
  ...
  it("fails a manifest changed after it was signed, and says why", async () => {
    ...
    const { code, stdout } = await hos(["manifest", "verify", "manifest.json", "--jwks", "jwks.json"], { files: { ...files, "manifest.json": changed } });
    expect(code).toBe(1);
    ...
    expect(stdout).toMatch(/✗ The manifest of https:\/\/housekeeping\.example fails verification\. A consumer treats it as no manifest: nothing it declares is processed\.\n$/);
  });
});
...
describe("hos manifest verify", () => {
  ...
  it("verifies a producer's manifest from its URL, with the keys it publishes on the same origin", async () => {
    ...
    const urls = { [`${base}manifest.json`]: manifestText, [`${base}manifest.jws`]: read("well-known/manifest.jws"), [`${base}jwks.json`]: read("well-known/jwks.json") };
    ...
    expect(expired.stdout).toContain("✗ https://housekeeping.example/.well-known/hos/manifest.jws: expired\n  error   The signature expired on 2027-09-26T00:00:00.000Z.");
  });

  it("reports a document it cannot fetch as a read error", async () => {
    const { code, stderr } = await hos(["manifest", "verify", "https://pms.example/.well-known/hos/manifest.json"], { urls: { "https://pms.example/.well-known/hos/manifest.json": manifestText } });
    expect(code).toBe(2);
  ...
  it.each([
    ...
    ["two key sets", ["verify", "manifest.json", "--jwks", "a.json", "--jwks-url", "https://a.example/jwks.json"], "give the key set once: --jwks or --jwks-url."],
    ["a time that is not one", ["verify", "manifest.json", "--jwks", "a.json", "--at", "soon"], "--at is soon; give a time such as 2026-11-01T00:00:00Z."],
