...
4. If contact is appropriate, update `last_contact_at`, set `retention_due_at` to `last_contact_at + interval '12 months'`, move to `contacted` and record the status change. The retention date is what the daily purge deletes on.
