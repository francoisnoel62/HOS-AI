...
The report keeps HOS ids, counts and room names. It never keeps Mews payloads or customer data. Nightly bookable services are taken as accommodation; `MEWS_SERVICE_IDS` overrides them. The logic is `lib/hos/mappings/mews-sync.ts`, on top of `lib/hos/mappings/sync.ts`, which the [Apaleo](../apaleo/README.md#live-check) and [Cloudbeds](../cloudbeds/README.md#live-check) live checks share. It is tested offline in `tests/unit/mews-sync.test.ts`.
