...
export type CloudbedsBlockWindow = { type: string; start_date: string; end_date: string; rooms: Array<{ room: string | null; starts_at: string | null; ends_at: string | null }> };
...
export function cloudbedsManifest(options: SyncOptions): ProducerManifest {
  return liveManifest(options, {
    ...
    limitations: ["Synchronised from get operations: reservation and room facts are dated when they were fetched, with the recorded time basis, and room blocks by the fetch."],
  });
}
...
export function syncCloudbeds(snapshot: CloudbedsSnapshot, options: SyncOptions): CloudbedsSyncReport {
  ...
  const windows = new Map(report.events.flatMap((event) => (event.type === "unit.maintenance_scheduled" ? [[event.data.maintenance_id, event.data] as const] : [])));
