...
export const pmsMappingCopy: Record<PmsMapping, PmsMappingCopy> = {
  mews: {
    ...
    findings: [
      ...
      ["Beds are units.", "Mews's demo enterprises assign dorm stays to beds, not rooms. On the first live run, those stays pointed to units HOS had never heard of. The adapter now publishes beds as units; a room and its beds are not yet one space in HOS."],
      ["Accommodation is a category, not a time unit.", "Parking and meeting rooms are sold by the day, and long stays by the month. The live check first took in the wrong services; the integration now picks accommodation by resource category."],
  ...
  apaleo: {
    ...
    findings: [
      ...
      ["Out of order needs a free room.", "On a live account, Apaleo refuses an OutOfOrder maintenance on a room already assigned to a reservation; only OutOfService is accepted. Both block the room's readiness in HOS."],
