  ...
  switch (kind) {
    case "founding_member":
      return {
        ...
        actorType: readString(formData, "actorType"),
        interestArea: readString(formData, "interestArea"),
        systemCategories: formData.getAll("systemCategories").filter((value): value is string => typeof value === "string"),
        engagementLevel: readString(formData, "engagementLevel"),
        futureRepresentatives: readString(formData, "futureRepresentatives"),
        website: readString(formData, "website"),
      };
    case "pilot":
      return {
        ...base,
        propertyCount: readString(formData, "propertyCount"),
        pms: readString(formData, "pms"),
        housekeepingTool: readString(formData, "housekeepingTool"),
        messagingTool: readString(formData, "messagingTool"),
        arrivalScenario: readString(formData, "arrivalScenario"),
        businessRole: readString(formData, "businessRole"),
        technicalRole: readString(formData, "technicalRole"),
        apiAccess: readString(formData, "apiAccess"),
      };
...
export function schemaKeyForRoute(kind: FormRouteKind): FormSchemaKey {
  return kind === "founding-member"
    ? "founding_member"
    : kind === "technical-contributor"
      ? "technical_contributor"
      : kind === "financial-patron"
        ? "financial_patron"
        : kind === "contact"
          ? "general_inquiry"
          : "pilot";
